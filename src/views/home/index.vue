...
export default {
  ...
  /**
   * 第一次进来页面加载完成时没有localstorage  this.navListActiveIndex为0   
   * 点击tab时 把this.navListActiveIndex切换为当前点击的index
   * 切换路由时设置一个key为home-index-nowIndex  value为当前this.navListActiveIndex的localstorage 并把当前scrollTop也保存
   * 下次再切回去时 get到home-index-nowIndex  把它的value赋给this.navListActiveIndex 获取到当前scrollTop 并把保存scrollTop的localstorage清除掉 就等于回到了上次离开的位置
   */
  mounted() {
